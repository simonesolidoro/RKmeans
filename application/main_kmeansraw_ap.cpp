...
int main() {
  std::string output_dir = "./output_kmeansraw_ap/";
  ...
  unsigned n_runs = 5; // 1 sola run perché mi interessa solo lo speedup
...
std::cout<<"tot oss: "<<responses.rows()<<" dim ogni oss:"<<responses.cols()<<std::endl;
  ...
  for (unsigned run = 0; run < n_runs; ++run) {
    ...
    std::string run_path = k_path +"/ap/run_" + std::to_string(run);
...
//RKMeans_parallel
    ...
    KMeans_par rkmeans(responses, dist_ap, init, k,
                    max_iter, seed);
