...
class fe_ls_separable_parallel {
  ...
  // hutch++
/*  std::optional<matrix_t> Shutch_;
  std::optional<matrix_t> Ghutch_;
  
    double edf(int r = 10, int seed = random_seed) {

      std::cout << "HUTCH++ " << r << std::endl;
    ...
    auto t1 = high_resolution_clock::now();
        ...
        if (!Shutch_.has_value() || r != Shutch_->rows()) {   // force reconstruction if r differs from old
...
// con queto no, anche se l'andamento del GCV è uguale, da capire perchè
            ...
            Shutch_ = matrix_t::Zero(n_locs_, r/3);
            for (int i = 0; i < n_locs_; ++i) {
                for (int j = 0; j < r/3; ++j) { 
                    //Us_->operator()(i, j) = rademacher(rng); 
                    Shutch_->operator()(i, j) = distribution(generator) ? 1.0 : -1.0;
                }
            }

            Ghutch_ = matrix_t::Zero(n_locs_, r/3);
            for (int i = 0; i < n_locs_; ++i) {
                for (int j = 0; j < r/3; ++j) { 
                    //Us_->operator()(i, j) = rademacher(rng); 
                    Ghutch_->operator()(i, j) = distribution(generator) ? 1.0 : -1.0;
                }
            }
...
// codice di prima
	...
	matrix_t AS(n_locs_, r/3);	
        for (int i = 0; i < r/3; ++i) {
            const auto& [f, g] = fit_(Shutch_->col(i), lambda_saved_->operator[](0), lambda_saved_->operator[](1));
            vector_t fn(n_ * m_);
	...
	std::cout << AS.topRows(5) << std::endl;
	std::cout << " --- " << std::endl;
	std::cout << AS.bottomRows(5) << std::endl;
	
	// compute an orthonormal basis Q for the span of AS
    auto t3 = high_resolution_clock::now();
	Eigen::FullPivHouseholderQR<matrix_t> qr(AS);
	matrix_t Q = qr.matrixQ();

    auto t4 = high_resolution_clock::now();
    auto ms_int2 = duration_cast<milliseconds>(t4 - t3);
    std::cout << "ms_int2: " << ms_int2.count() << "s" << std::endl;

    auto t5 = high_resolution_clock::now();

    
	matrix_t AQ(n_locs_, r/3);
        for (int i = 0; i < r/3; ++i) {
            const auto& [f, g] = fit_(Q.col(i), lambda_saved_->operator[](0), lambda_saved_->operator[](1));
            vector_t fn(n_ * m_);
            for (int i = 0; i < m_; ++i) { fn.middleRows(i * n_, n_) = Psi_ * f.middleRows(i * n_dofs_, n_dofs_); }
            AQ.col(i) = fn;
        }
        double trQAQ = 0;
        for (int i = 0; i < r / 3; ++i) { trQAQ += Q.col(i).dot(AQ.col(i)); }
	
	std::cout << "AQ" << std::endl;
	std::cout << AQ.topRows(5) << std::endl;
	std::cout << " --- " << std::endl;
	std::cout << AQ.bottomRows(5) << std::endl;
	std::cout << "trQAQ: " << trQAQ << std::endl;

    auto t6 = high_resolution_clock::now();
    auto ms_int3 = duration_cast<milliseconds>(t6 - t5);
    std::cout << "ms_int3: " << ms_int3.count() << "s" << std::endl;

    auto t7 = high_resolution_clock::now();
	
        matrix_t G = (*Ghutch_) - Q * (Q.transpose() * (*Ghutch_));
	matrix_t AG(n_locs_, r/3);
        for (int i = 0; i < r/3; ++i) {
            const auto& [f, g] = fit_(G.col(i), lambda_saved_->operator[](0), lambda_saved_->operator[](1));
            vector_t fn(n_ * m_);
            for (int i = 0; i < m_; ++i) { fn.middleRows(i * n_, n_) = Psi_ * f.middleRows(i * n_dofs_, n_dofs_); }
            AG.col(i) = fn;
        }
        ...
        double trGAG = 0;
        for (int i = 0; i < r / 3; ++i) { trGAG += G.col(i).dot(AG.col(i)); }

	std::cout << "trGAG: " << trGAG << std::endl;

    auto t8 = high_resolution_clock::now();

    auto ms_int4 = duration_cast<milliseconds>(t8 - t7);
    std::cout << "edf: " << (trQAQ + 3.0/r * trGAG) << std::endl; 
    std::cout << ms_int4.count() << "s" << std::endl;
	
        return trQAQ + 3.0/r * trGAG;
    }
*/
