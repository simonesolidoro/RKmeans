...
int main() {
  std::string output_dir = "./output_presmoothDF_kmean_st/";
  ...
  unsigned n_runs = 10; // 1 sola run perché mi interessa solo lo speedup
  ...
  lambda.push_back(std::pow(10, -16.8 ));
  lambda.push_back(std::pow(10, -16.8 ));
...
std::cout<<"dist e init creati"<<std::endl;
  ...
  //std::string resp_file = data_dir + "dataset.csv"; presmoooth_jb_1685_dataset_cntrl_scz_0_124.csv
  std::string resp_file = data_dir + "presmoooth_jb_1685_dataset_cntrl_scz_0_124.csv";
  Eigen::MatrixXd responses =
      csv2mat<double>(resp_file, 0, 0);
//togliamo outlier di distanza st
...
std::cout<<"tot oss: "<<responses.rows()<<" dim ogni oss:"<<responses.cols()<<std::endl;
  ...
  for (unsigned run = 0; run < n_runs; ++run) {
    ...
    std::string run_path = k_path +"/st/run_" + std::to_string(run);
...
//RKMeans_parallel
    // RKMeans_neuro_old rkmeans(dist, init, D,T, fe_ls_separable_parallel(std::pair {a, F}, 500, 1e-9), responses, k, max_iter, seed); 
    KMeans rkmeans(responses, dist_st, init, k,
                    max_iter, seed);
