...
//script per regKmeans con ap 
{
  std::string output_dir = "./output_jb_regKmeans_ap_noout/";
...
v_init_runs.push_back({i+80,i+140});
}
...
std::cout<<"tot oss: "<<responses.rows()<<" dim ogni oss:"<<responses.cols()<<std::endl;
  ...
  for (unsigned run = 0; run < n_runs; ++run) { //n_runs
    ManualInitPolicy init(v_init_runs[run]);
    std::string run_path = output_dir +"ap/run_" + std::to_string(run);
...
//RKMeans_parallel
        RKMeans_par rkmeans(dist_ap, init, D,T,
