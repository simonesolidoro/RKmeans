...
int main() {
	{  std::string output_dir = "./output_jb_regKmeans_femfemk_noout/";
...
//  lambda.push_back(std::pow(10, -5 ));
 ...
 std::cout << T.n_nodes() << std::endl;
  ...
  // 1D
  FeSpace Vht(T, P1<1>);
  TrialFunction ft(Vht);
  TestFunction vt(Vht);


 auto masst = integral(T)(ft*vt);
  auto T0 = masst.assemble();
 auto mass = integral(D)(f * v);
  auto P0 = mass.assemble();

  auto femfemk = kronecker(T0,P0);
//L2Policy_spaziotempo dist_st(P0,25150,129);
L2Policy dist_femfemk(femfemk);
...
std::cout<<"tot oss: "<<responses.rows()<<" dim ogni oss:"<<responses.cols()<<std::endl;
  ...
  for (unsigned run = 0; run < n_runs; ++run) {
    ...
    std::string run_path = output_dir +"femfemk/run_" + std::to_string(run);
...
//RKMeans_parallel
        RKMeans rkmeans(dist_femfemk, init, D,T,
