...
class TpSpace {
    ...
    class dof_handler_t {
       public:
        ...
        std::array<index_t, tp_order> n_dofs_local_per_cell() const {
            std::array<index_t, tp_order> dofs_;
            int i = 0;
            std::apply(
              [&](auto... ts) {
                  (
                    [&]() {
                        dofs_[i] = ts->n_dofs_per_cell();
                        i++;
                    }(),
                    ...);
              },
        ...
        // compute global dof numbering on active cell identified by multi-index (idx_1, idx_2, ..., idx_{tp_order})
        template <typename... IndexType>
            requires(sizeof...(IndexType) == tp_order) && (std::is_convertible_v<IndexType, index_t> && ...)
        std::pair<std::array<std::vector<index_t>, tp_order>, std::vector<index_t>>
        active_dofs(IndexType&&... idxs) const {
        ...
        std::pair<std::array<std::vector<index_t>, tp_order>, std::vector<index_t>>
        active_dofs(const IndexType& idx) const {
       ...
       private:
        template <typename IndexType>
            requires(internals::is_subscriptable<IndexType, index_t>)
        std::array<std::vector<index_t>, tp_order> local_dofs_(const IndexType& idx) const {
            std::array<std::vector<index_t>, tp_order> local_dofs;
            index_t i = 0;
            std::apply(
              [&](auto... ts) {
                  (
                    [&]() {
                        ts->active_dofs(idx[i], local_dofs[i]);
                        i++;
                    }(),
                    ...);
              },
        ...
        std::vector<index_t> local_to_global_(const std::array<std::vector<index_t>, tp_order>& local_dofs) const {
            ...
            for (int i = 0; i < n_dofs_per_cell(); ++i) {
                tp_dofs.push_back(internals::apply_index_pack<tp_order>([&]<int... Ns_>() {
                    return ((local_dofs[Ns_][multi_index[Ns_]] * offset_[Ns_]) + ... + 0);
                }));
                // increase multi-index
                multi_index[0]++;
                int j = 0;
        ...
        template <typename IndexType>
            requires(internals::is_subscriptable<IndexType, index_t>)
        std::pair<std::array<std::vector<index_t>, tp_order>, std::vector<index_t>>
        dof_tensorize_(const IndexType& idx) const {
            std::array<std::vector<index_t>, tp_order> local_dofs;
            std::vector<index_t> tp_dofs;
            if constexpr (tp_order == 2) {   // optimized case for order 2 tensor product
                std::get<0>(tp_dof_handler_)->active_dofs(idx[0], local_dofs[0]);
                std::get<1>(tp_dof_handler_)->active_dofs(idx[1], local_dofs[1]);
                index_t n_lhs_dofs = local_dofs[0].size();
                index_t n_rhs_dofs = local_dofs[1].size();
                // perform dof tensor product
                tp_dofs.reserve(n_lhs_dofs * n_rhs_dofs);
                int lhs_offset = std::get<0>(tp_dof_handler_)->n_dofs();
                for (int i = 0; i < n_rhs_dofs; ++i) {
                    for (int j = 0; j < n_lhs_dofs; ++j) {
                        tp_dofs.push_back(local_dofs[0][j] + local_dofs[1][i] * lhs_offset);
                    }
                }
                return std::make_pair(local_dofs, tp_dofs);
            } else {
                // BUG
                local_dofs = local_dofs_(idx);
                tp_dofs = local_to_global_(local_dofs);
                return std::make_pair(local_dofs, tp_dofs);
            }
        }
...
class TpFunction :
   ...
   public:
    ...
    std::vector<OutputType> grid_eval(InputType_&&... grids) {
        ...
        std::array<std::vector<Scalar>, tp_order> shape_values;   // evaluation of basis system at grid points, cambia nome
	...
	// initialization
        internals::for_each_index_and_args<tp_order>(
          [&]<int Ns_, typename InputType__>(const InputType__& grid) {
              ...
              inner_limits[Ns_] = function_space.dof_handler().n_dofs_per_cell();
              ...
              shape_values[Ns_].resize(n_points * inner_limits[Ns_]);
              for (index_t i = 0; i < n_points; ++i) {
                  for (index_t j = 0; j < inner_limits[Ns_]; ++j) {
                      shape_values[Ns_][i * inner_limits[Ns_] + j] = function_space.eval_cell_value(
        ...
        for (index_t i = 0; i < tp_order; ++i) { cell_ids[i] = grid_ids[i][outer_index[i]]; }
	index_t i = 0;
        while (outer_index) { // loops over evaluation points
            const auto& [local_dofs, global_dofs] = tp_space_->dof_handler().active_dofs(cell_ids);
            inner_index.reset();
            // evaluate tensor basis expansion
            for (index_t j = 0, n = global_dofs.size(); j < n; ++j, ++inner_index) { // loops over active dofs
                result[i] +=
                  coeff_[global_dofs[j]] * internals::apply_index_pack<tp_order>([&]<int... Ns_>() {
                      return (shape_values[Ns_][outer_index[Ns_] * inner_limits[Ns_] + inner_index[Ns_]] * ... * 1.0);
                  });
            }
            ++outer_index;
