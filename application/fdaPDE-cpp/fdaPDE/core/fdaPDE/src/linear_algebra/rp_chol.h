   ...
   public:
	...
	double err = norm;
        ...
        while (i < max_iter * block_sz_ && err > tol * norm) {
            ...
            matrix_t G = A(Eigen::placeholders::all, pivot_vec) -
                         L_.leftCols(i) * L_(pivot_vec, Eigen::placeholders::all).leftCols(i).transpose();
            // compute stabilized cholesky
            double shift = std::numeric_limits<double>::epsilon() * G(pivot_vec, Eigen::placeholders::all).trace();
            chol_t chol(
              G(pivot_vec, Eigen::placeholders::all) + shift * matrix_t::Identity(pivot_vec.size(), pivot_vec.size()));
            // update step
