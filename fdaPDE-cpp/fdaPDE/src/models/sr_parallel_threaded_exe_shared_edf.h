...
class SRPDE {
   ...
   public:
    ...
    vector_t fitted(int worker_id = 0) const {
        vector_t fitted_ = solver_.fn(worker_id); //fn è const in solver 
    ...
    struct gcv_t : public ScalarFieldBase<n_lambda, gcv_t> {
        ...
        gcv_t(SRPDE* model, const std::vector<edf_cache_t>& edf_cache) :
            model_(model),
            n_(model->n_obs()),
            q_(model->n_covs()),
            ...
            r_(100),
            seed_(random_seed) { }
        gcv_t(SRPDE* model, const std::vector<edf_cache_t>& edf_cache, int r, int seed) :
            model_(model), n_(model->n_obs()), q_(model->n_covs()), edf_cache_(edf_cache), r_(r), seed_(seed) {}
        gcv_t(SRPDE* model) : gcv_t(model, std::vector<edf_cache_t>(1)) { }
        gcv_t(SRPDE* model, int r, int seed) : gcv_t(model, std::vector<edf_cache_t>(1), r, seed) { }
    ...
    struct gcv_par_t : public ScalarFieldBase<n_lambda, gcv_par_t> {
        ...
        gcv_par_t(SRPDE* model, const edf_cache_t& edf_cache) :
            model_(model),
            n_(model->n_obs()),
            q_(model->n_covs()),
            edf_cache_(edf_cache),
            r_(100),
            seed_(random_seed) { }
        gcv_par_t(SRPDE* model, const edf_cache_t& edf_cache, int r, int seed) :
            model_(model), n_(model->n_obs()), q_(model->n_covs()), edf_cache_(edf_cache), r_(r), seed_(seed) {}
        gcv_par_t(SRPDE* model) : gcv_par_t(model, edf_cache_t()) { }
        gcv_par_t(SRPDE* model, int r, int seed) : gcv_par_t(model, edf_cache_t(), r, seed) { }
        ...
        template <typename... LambdaT>
            requires(std::is_convertible_v<LambdaT, double> && ...) && (sizeof...(LambdaT) == StaticInputSize)
        constexpr double operator()(LambdaT... lambda) {
            if(!ready_per_parallelo.load(std::memory_order_relaxed)){
                std::lock_guard<std::shared_mutex> lock(m_gcv_);
            ...
            double edf_tmp;
            {
                std::shared_lock<std::shared_mutex> slock (m_gcv_); //lock lettura codiviso 
                auto it = edf_cache_.find(lambda_vec);
                if (it != edf_cache_.end()) {// se esiste già
                    edf_tmp = it->second;}
                else{
                    slock.unlock(); // passare a lock in scrittura
                    edf_tmp = model_->edf(r_, seed_, worker_id); //calcolo 

                    std::unique_lock<std::shared_mutex> lock(m_gcv_); //lock scrittura
                    edf_cache_[lambda_vec] = edf_tmp; //agioro
                }// rilascio lock scrittura 
            }// rilascia lock lettura se non già fatto
       ...
       private:
        ...
        // stochastic edf approximation parameter
        int r_, seed_;
        std::atomic<bool> ready_per_parallelo = false; //atomico ? credo di si
        std::shared_mutex m_gcv_;
    };
    gcv_par_t gcv_par() { return gcv_par_t(this); }
    gcv_par_t gcv_par(const gcv_par_t::edf_cache_t& edf_cache) { return gcv_par_t(this, edf_cache); }
    gcv_par_t gcv_par(int r, int seed) { 
        return gcv_par_t(this, r, seed); }
    gcv_par_t gcv_par(const gcv_par_t::edf_cache_t& edf_cache, int r, int seed) { return gcv_par_t(this, edf_cache, r, seed); }
