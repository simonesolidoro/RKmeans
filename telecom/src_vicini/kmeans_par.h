...
// RKMeans class for regularized KMeans per spazio tempo con jacobi GCV OTTIMIZZAZIONE PER OGNI REGULIRIZE_CENTROIDS ATTIVA
// RKMeans class for regularized KMeans
template <typename DistancePolicy, typename InitPolicy, typename Triangulation,typename Triangulationt,
          typename Penalty>
class RKMeans_gcv {
  ...
  void regularize_centroids(std::optional<std::vector<double>> lambda = std::nullopt) {
    for (unsigned c = 0; c < k_; ++c) {
      ...
      if (lambda) {
        ...
        // //   optimizer.optimize(model.gcv(edf_cache, 100, seed), lambda_grid_);
        // // }
        auto gcv = model.gcv(edf_cache,100, seed);
        optimizer.optimize(gcv, lambda_grid_);
        // gcv.edf_cache().clear();
...
public:
  RKMeans_gcv(const DistancePolicy &dist, const InitPolicy &init_policy,
          const Triangulation &triang,const Triangulationt &triangt, const Penalty &penalty,
          const Eigen::MatrixXd &Y, unsigned k = 3,
          unsigned max_iter = MAX_KMEANS_ITERATIONS,
          std::optional<unsigned> seed = std::nullopt)
...
std::cout<<std::endl;
  ...
  void set_gcv_grid(Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic,Eigen::RowMajor> grid) { 
	  lambda_grid_.resize(grid.rows(), grid.cols());
	  lambda_grid_ = grid; }
...
// RKMeans class for regularized KMeans per spazio tempo con jacobi GCV OTTIMIZZAZIONE PER OGNI REGULIRIZE_CENTROIDS ATTIVA
// per monolitica con Bspline. (diversa da uso df perché uso gcv_par)
// RKMeans class for regularized KMeans
template <typename DistancePolicy, typename InitPolicy, typename Triangulation,typename Triangulationt,
          typename Penalty>
class RKMeans_gcv_mono {
private:
  const Eigen::MatrixXd &Y_;
  DistancePolicy dist_;
  InitPolicy init_policy_;
...
using edf_cache_t = std::unordered_map<std::array<double, 2>, double, internals::std_array_hash<double, 2>>;
  ...
  void regularize_centroids(std::optional<std::vector<double>> lambda = std::nullopt) {
	...
	edf_cache = gcv.edf_cache(); //aggiorno membro edf_cache di kmeans. // fatto bene sarebbe aggiornarlo a prima iterazione di kmeans.
      ...
      centroids_.row(c) = model.fitted(); //importante fitted non f() che ha dim diversa 
    }
  };

public:
  RKMeans_gcv_mono(const DistancePolicy &dist, const InitPolicy &init_policy,
          const Triangulation &triang,const Triangulationt &triangt, const Penalty &penalty,
          const Eigen::MatrixXd &Y, unsigned k = 3,
          unsigned max_iter = MAX_KMEANS_ITERATIONS,
          std::optional<unsigned> seed = std::nullopt)
