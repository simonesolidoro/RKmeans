...
int main() {
  ...
  std::string output_dir = "./output_jb/";
...
std::cout<<"create griglie gcv"<<std::endl; 
   ...
   for (std::string_view dir1 : {"2d_st_smooth_fitted_ap","2d_st_smooth_fitted_st","2d_st_regk_ap","2d_st_regk_st"})
...
//std::cout<<"prodotto Kron per massa spazio tempo fatto"<<std::endl; 
  ...
  L2Policy_st_ap dist_2d_ap(R0_2d,R0_2d.rows(),istanti.rows());
  L2Policy_st_ap dist_3d_ap(R0_3d,R0_3d.rows(),istanti.rows());
...
// presmoot ma con jacobi parallel quindi dataset di fitted() cosi da fare kmeans normale ma con dist ap 

  for (auto &dir1 : {"2d_st_smooth_fitted_ap"}) { //, "3d_st_smooth_fitted" per ora solo 2d
...
//std::cout<<"crea file mem centr"<<std::endl; 
      ...
      std::string dirdata = (dir1 == "2d_st_smooth_fitted_ap")? "2d_st":"3d_st";
      ...
      //presmoothing 
      if(dir1 == "2d_st_smooth_fitted_ap"){
...
//std::cout<<"ottimo lambda:"<<optimizer.optimum()[0]<<" "<<optimizer.optimum()[1]<<std::endl;
  //        model.fit(0,optimizer.optimum()[0],optimizer.optimum()[1]); 5.62341e-08 5.62341e-06
  	  model.fit(5.62341e-08, 5.62341e-08);
...
//      std::cout<<"f.cols()"<<f_.cols()<<" r:"<<f_.rows()<<std::endl;
      ...
      std::cout<<"fine presmooth ap"<<std::endl;     
      unsigned n_iter;
      ...
      if (dir1 == "2d_st_smooth_fitted_ap") {
        ...
        //                 max_iter, seed);
        KMeans rkmeans(responses_smooth,dist_2d_ap, init_manual, k,
                        max_iter, seed);
      ...
      ss << dir1 << "/" << curve_types[0] << "_" << n << ": "
          ...
          << " iterations (max=" << max_iter << "), time (pre-smooth(jb,fitted)+kmeans(con dist ap)):" << elapsed_time;
...
// rifare presmooth non ha senso ma tanto runna stanotte, poi sistemalo
// presmoot ma con jacobi parallel quindi dataset di fitted() cosi da fare kmeans normale ma con dist st 

  for (auto &dir1 : {"2d_st_smooth_fitted_st"}) { //, "3d_st_smooth_fitted" per ora solo 2d
    for (unsigned n = 0; n < N; ++n) {
//std::cout<<"crea file mem centr"<<std::endl; 
      std::string out_memb_file = output_dir + dir1 + "/" + curve_types[0] + "/memberships"+ "_" + std::to_string(n) + ".csv";
      std::string out_cent_file = output_dir + dir1 + "/" + curve_types[0] + "/centroids"+ "_" + std::to_string(n) + ".csv";
      ...
      std::string dirdata = (dir1 == "2d_st_smooth_fitted_st")? "2d_st":"3d_st";
      std::string resp_file = std::string("./../data/") + dirdata + "/" + curve_types[0] + "/" + curve_types[0] + "_" + std::to_string(n) + ".csv"; // curve_types[0] perche un solo curve type Funzione_test.
      Eigen::MatrixXd responses = csv2mat<double>(resp_file);
      Eigen::MatrixXd responses_smooth(responses.rows(),responses.cols());
      ...
      t1 = high_resolution_clock::now();
      //presmoothing 
      if(dir1 == "2d_st_smooth_fitted_st"){
//std::cout<<"smoothing resp"<<std::endl; 
        for (int s=0 ; s< responses.rows(); s++){
//std::cout<<"smooth s:"<<s<<std::endl;
          // load data in geoframe
...
//      std::cout<<"f.cols()"<<f_.cols()<<" r:"<<f_.rows()<<std::endl;
      ...
      ss << dir1 << "/" << curve_types[0] << "_" << n << ": "
          << kmeans_type << " execution completed in " << n_iter
          << " iterations (max=" << max_iter << "), time (pre-smooth(jb,fitted)+kmeans(con dist st)):" << elapsed_time;
      std::string msg = ss.str();
      std::cout << msg << std::endl;
      file_log << msg << std::endl;
...
// kmenas regolarizzato su dataset originale (distanza AP)
  ...
  for (auto &dir1 : {"2d_st_regk_ap"}) { //, "3d_st_regk" per ora solo 2d
    for (unsigned n = 0; n < N; ++n) {
//std::cout<<"crea file mem centr"<<std::endl; 
      std::string out_memb_file = output_dir + dir1 + "/" + curve_types[0] + "/memberships"+ "_" + std::to_string(n) + ".csv";
      std::string out_cent_file = output_dir + dir1 + "/" + curve_types[0] + "/centroids"+ "_" + std::to_string(n) + ".csv";
      ...
      std::string dirdata = (dir1 == "2d_st_regk_ap")? "2d_st":"3d_st";
      std::string resp_file = std::string("./../data/") + dirdata + "/" + curve_types[0] + "/" + curve_types[0] + "_" + std::to_string(n) + ".csv"; // curve_types[0] perche un solo curve type Funzione_test.
      Eigen::MatrixXd responses = csv2mat<double>(resp_file);
      ...
      if (dir1 == "2d_st_regk_ap") {
//	std::cout<<" dentro if di 2d_st_reg_nosmooth"<<std::endl;
        // RKMeans
	int max_it_ = 15;
        RKMeans rkmeans(dist_2d_ap, init_manual, D2,T,
...
//	std::cout<<"creato rkmeans2d"<<std::endl;
        ...
        //                max_iter, seed);
        //rkmeans.set_gcv_grid(lambda_2d); //ora lambda scelto, poi se passato lambda grid va sistemato perche RKMeans prende vector<vector> per lambda grid
//	std::cout<<"set griglia 2d"<<std::endl;
      ...
      ss << dir1 << "/" << curve_types[0] << "_" << n << ": "
          << kmeans_type << " execution completed in " << n_iter
          << " iterations (max=" << max_iter << "), time (reg-kmeans jb, distnza AP no Kron):" << elapsed_time;
      std::string msg = ss.str();
      std::cout << msg << std::endl;
      file_log << msg << std::endl;
...
// kmenas regolarizzato su dataset originale (distanza ST)
  ...
  for (auto &dir1 : {"2d_st_regk_st"}) { //, "3d_st_regk" per ora solo 2d
    for (unsigned n = 0; n < N; ++n) {
//std::cout<<"crea file mem centr"<<std::endl; 
      std::string out_memb_file = output_dir + dir1 + "/" + curve_types[0] + "/memberships"+ "_" + std::to_string(n) + ".csv";
      std::string out_cent_file = output_dir + dir1 + "/" + curve_types[0] + "/centroids"+ "_" + std::to_string(n) + ".csv";
      ...
      std::string dirdata = (dir1 == "2d_st_regk_st")? "2d_st":"3d_st";
      std::string resp_file = std::string("./../data/") + dirdata + "/" + curve_types[0] + "/" + curve_types[0] + "_" + std::to_string(n) + ".csv"; // curve_types[0] perche un solo curve type Funzione_test.
      Eigen::MatrixXd responses = csv2mat<double>(resp_file);
      t1 = high_resolution_clock::now();
