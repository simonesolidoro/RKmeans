...
//sin(2*pi*(coe(y,1)*x*cos(9/5*t/tf-2)-y*sin(9/5*t/tf-2)))*cos(2*pi*(coe(y,1)*x*cos(9/5*t/tf-2+pi/2)+coe(x,1)*y*sin((9/5*t/tf-2)*pi/2)))
  ...
  // mesh
  Triangulation<2, 2> D2("./src/mesh_02/nodes.csv", "./src/mesh_02/cells.csv", "./src/mesh_02/boundary_nodes.csv", false,false);
  std::cout<<"caricata mesh 02"<<std::endl;
  // salva mesh
  ...
  for (unsigned n = 0; n<N_st ; n++){
    //2D spazio-tempo separabile 
    {
      ...
      for (std::size_t i = 0; i < n_obs_per_clust_st; ++i) {
        double dt = 0.1; //di quando sono shiftati i due centroidi nel tempo
        for (unsigned tt = 0; tt < T_nodi; ++tt) {
          ...
          for (std::size_t j = 0; j < n_nodes; ++j) {
            ...
            if (i == 0) {
              ...
              cent_no_noise(1, idx) = f(x,y,t,dt);
            }

            out_st(i, idx)                       = f(x,y,t,0) + noise(gen);
            out_st(i + n_obs_per_clust_st, idx)      = f(x,y,t,dt) + noise(gen);
          }
        }
      }
