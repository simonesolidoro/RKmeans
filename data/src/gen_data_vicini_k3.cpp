...
double deltat= 0.03;
  ...
  std::size_t n_obs_per_clust_st = 20;//20; //per test solo 4 poi simulazioni vere aumenta a 30
  std::size_t n_obs_st = n_obs_per_clust_st * k_st;
  // noise
  std::mt19937 gen(seed);
  std::normal_distribution<double> noise(0.0, std::sqrt(var));
  ...
  for (unsigned n = 0; n<N_st ; n++){
    //2D spazio-tempo separabile 
    {
      ...
      for (std::size_t i = 0; i < n_obs_per_clust_st; ++i) {
        double dt1 = deltat; //di quando sono shiftati i due centroidi nel tempo
	double dt2 = 2*deltat;
